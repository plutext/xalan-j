    ...
    for(int i = 0; i < nMethods; i++)
    {
      ...
      if(method.getName().equals(name))
      {
        ...
        if ( (paramTypes.length == 2)
           && paramTypes[1].isAssignableFrom(org.docx4j.org.apache.xalan.templates.ElemExtensionCall.class)
                                         && paramTypes[0].isAssignableFrom(org.docx4j.org.apache.xalan.extensions.XSLProcessorContext.class) )
        {
          if ( ++bestScoreCount == 1 )
            bestMethod = method;
          else
      ...
      default:
        ...
        else                                  // must be an element call
        {
          return resultString + classObj.getName() + "." + funcName
                    + "(org.docx4j.org.apache.xalan.extensions.XSLProcessorContext, "
